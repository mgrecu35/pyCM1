...
for i in range(3):
    ...
    #plt.colorbar()
    plt.tight_layout()
    plt.savefig('correctionTableDPR_Land_%s.png'%st[i])
mProfS/=(cS+1e-9)
for i in range(46):
...
for i in range(-3):
    ...
    plt.title("Land %s"%st[i])
    #plt.colorbar()
    plt.savefig('resampleD_DPR_%s_Land.png'%st[i])
...
ax.bar(np.arange(3)+w,(systDiff[:,2]-systDiff[:,0])/systDiff[:,0]-0.01, 2*w, label='corrected')
...
plt.colorbar()
plt.tight_layout()
plt.savefig('LandPersistenceHist2d.png')
...
plt.colorbar()
plt.tight_layout()
plt.savefig('LandVPP_Hist2d.png')
