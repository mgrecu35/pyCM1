     ...
     subroutine cm1_init(ibo,ieo,jbo,jeo,kbo,keo,&
...
!-----------------------------------------------------------------------------
      ...
      implicit none
      integer, intent(out) :: ibmo,iemo,jbmo,jemo,kbmo,kemo,numqo
...
!----------------------------------------------------------------------
      ...
      if( myid.eq.0 )then
        ...
        print *,'|---------------------------------------------------------------|'
        print *,'|                                                               |'
        print *,'|          You are using CM1, Release 19.10 (cm1r19.10)         |'
        print *,'|                                                               |'
        print *,'|---------------------------------------------------------------|'
        print *
      endif
...
!  Now, allocate the mother lode, then call INIT3D
      ...
      qke = 0.0
      allocate(   tke_pbl(ibmynn:iemynn,jbmynn:jemynn,kbmynn:kemynn) )
      tke_pbl = 0.0
      ...
      nupdraft = 0
      allocate( ktop_shallow(ibmynn:iemynn,jbmynn:jemynn) )
      ktop_shallow = 0
...
!  Prepare for I/O:
      ...
      call       setup_output(qname,qunit,budname,   &
...
!----------------------------------------------------------------------
      ...
      IF(irst.eq.1)THEN
        ...
        call     read_restart(nstep,srec,sirec,urec,vrec,wrec,nrec,prec,           &
                              ...
                              tdiag,qdiag,phi1,phi2,qke,tsq,qsq,cov,sh3d,el_pbl,   &
...
122     format(1x,'cflmax,ksmax,dt,nsound:',2x,f6.4,2x,f6.4,2x,f9.4,2x,i3)
        ...
        call     solve(nstep,rbufsz,num_soil_layers,                  &
                   ...
                   out2d,out3d,                                      &
                   dowriteout,dorad,getdbz,getvt,dotdwrite,dotbud,   &
      ...
      getpbl = .true.
      if( restarted .and. (ipbl.eq.4.or.ipbl.eq.5) ) getpbl = .false.
      ...
      call sfc_and_turb(getsfc,getpbl,nstep,dt,dosfcflx,cloudvar,qbudget,   &
                   ...
                   tsk,thflux,qvflux,cd,ch,cq,u1,v1,s1,t1,tlh,f2d,   &
                   dum1,dum2,dum3,dum4,dum5,dum6,dum7,dum8,          &
                   divx,rho,rr,rf,prs,                               &
                   t11,t12,t13,t22,t23,t33,                          &
                   ...
                   nm,defv,defh,lenscl,dissten,radsw,radswnet,radlwin, &
                   thpten,qvpten,qcpten,qipten,upten,vpten,qnipten,qncpten, &
                   xkzh,xkzq,xkzm,                                   &
                   tsq,qsq,cov,sh3d,el_pbl,qc_bl,cldfra_bl,                  &
                   qWT,qSHEAR,qBUOY,qDISS,dqke,qke_adv,qke,                  &
                   tke_pbl,edmf_a,edmf_w,edmf_qt,edmf_thl,edmf_ent,edmf_qc,  &
                   vdfg,maxmf,nupdraft,ktop_shallow,                         &
                   lwten,swten,tdiag,                                &
                   ...
                   mznt,swspd,smois,taux,tauy,hpbl2d,evap2d,heat2d,  &
                   ...
                   uw31,uw32,ue31,ue32,us31,us32,un31,un32,          &
                   kw1,kw2,ke1,ke2,ks1,ks2,kn1,kn2,                  &
                   flag  ,out2d,out3d,rtime,update_sfc,dotbud)
        ! end_sfc_and_turb
...
110       format(2x,i12,4x,f18.6,a5)
      ...
      IF(output_format.eq.1.or.output_format.eq.2)THEN
        ...
        DO n=1,nn
          ...
          call writeout(frec,urec,vrec,wrec,rtime,dt,fnum,nwrite,qname,                        &
                        ...
                        mznt,taux,tauy,                                                        &
                        tsq,qsq,cov,sh3d,el_pbl,qc_bl,cldfra_bl,                               &
                        qWT,qSHEAR,qBUOY,qDISS,dqke,qke_adv,qke,tke_pbl,                       &
    ...
    IF( doturbdiag )THEN
      if( dotdwrite )then
        ...
        call   turbdiag(nstep,mtime,nwritet,trecs,trecw,qname,qunit,dt,dosfcflx, &
                   xh,rxh,arh1,arh2,uh,ruh,xf,rxf,arf1,arf2,uf,ruf,            &
                   yh,vh,rvh,yf,vf,rvf,                                        &
                   ...
                   nm,defv,defh,lenscl,dissten,pt3d,                           &
